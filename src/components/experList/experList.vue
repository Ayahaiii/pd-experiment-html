        ...
        <div class="content-box">
            ...
            <div class="list" v-for="(item,index) in showList" :key="index">
                ...
                <el-row>
                    ...
                    <el-col :span="2">
                        <el-button type="primary" @click="toDelete(item.id)">删除</el-button>
                    </el-col>
                </el-row>
            </div>
            <!-- 页显示 -->
...
export default {
  ...
  methods: {
      ...
      getList(pageNum,pageSize){
        ...
        this.httpPost("pd/v1/getUserTaskList", params).then(res => {
            if (res && res.code === 0) {
      ...
      toDetail(item){
        //当前用户实验id存到
        localStorage.setItem("userTaskId",item.id);
        ...
        let path = '/pd/stepIntro/'+item.taskId;
      ...
      toDelete(id){
        this.$confirm('此操作将永久删除该实验, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.httpPost("/pd/v1/delUserTask", params).then(res => {
                if (res && res.code === 0) {
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
